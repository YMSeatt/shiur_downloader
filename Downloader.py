...
from tkinter import messagebox, ttk, simpledialog # Added simpledialog for range input (alternative)
import uuid
import logging
import subprocess
...
class MasechetDownloader:
    ...
    def daf_amud_calculator(page_number):
        ...
        return daf, amud
    def daf_amud_calculator2(page_number):
        # ... (daf_amud_calculator code remains the same) ... [source: 2]
        daf = 3 + (page_number - 3) // 2
        amud = "א" if page_number % 2 != 0 else "ב"
        return daf, amud
    ...
    def __init__(self,root):
        ...
        container.grid(sticky=('N', 'E', 'S','W'),column=1)
        ...
        top_controls_frame.grid(sticky=("N","W","E"), padx=0,column=0,row=0, pady=5)
        ...
        self.content_canvas.grid(column=0,sticky=('W','N','E','S'),row=1,rowspan=3,columnspan=4)
        ...
        self.content_canvas.bind("<Configure>", lambda e: self.content_canvas.grid_configure(rowspan=1,sticky=("N","S","E","W")))
        # --- End Scrolling Fix ---
        ...
        ttk.Radiobutton(select_type_frame, text="Dapim", variable=self.select_type_var, value="Dapim", command=self.update_individual_list).grid(column=2, padx=5,row=2,sticky="W")#left
        ttk.Radiobutton(select_type_frame, text="Amudim", variable=self.select_type_var, value="Amudim", command=self.update_individual_list).grid(column=1, padx=5,row=2,sticky="W")#left


        # --- Selection Mode (All/Range/Individual) --- (In Scroll Frame)
        self.selection_mode_var = tk.StringVar(value="All") # Default to All
        ttk.Label(self.content_canvas, text="Selection Mode:").grid(column=1,padx=20,pady=(10, 0))#
        mode_frame = ttk.Frame(self.content_canvas)
        mode_frame.grid(column=1)#
        ttk.Radiobutton(mode_frame, text="All", variable=self.selection_mode_var, value="All", command=self.toggle_selection_widgets).grid(column=1,row=3, padx=5)#left
        ttk.Radiobutton(mode_frame, text="Range", variable=self.selection_mode_var, value="Range", command=self.toggle_selection_widgets).grid(column=2,row=3, padx=5)#left
        ttk.Radiobutton(mode_frame, text="Individual", variable=self.selection_mode_var, value="Individual", command=self.toggle_selection_widgets).grid(column=3,row=3, padx=5)#left
        ...
        self.range_end_spin.bind(self.validate_range)
        ...
        ttk.Label(self.individual_list_frame_container, text="Select Items:").grid(column=1,row=3)

        # Inner frame for the actual list, inside a canvas for scrolling
        self.individual_canvas = tk.Canvas(self.individual_list_frame_container, height=150, borderwidth=1, relief="sunken")
        ...
        self.individual_scrollbar.grid(sticky=("N","S","W"),rowspan=3,column=2)
        self.individual_canvas.grid(sticky=('N','W'),column=1,row=4 )#left
        ...
        ttk.Label(self.content_canvas, text="Merge Options:").grid(pady=(15, 0),column=1)
        self.merge_options_frame = ttk.Frame(self.content_canvas)
        self.merge_options_frame.grid(column=1,row=5, padx=20) # Align left slightly for clarity

        self.merge_into_one_pdf_var = tk.BooleanVar(value=False)
        self.merge_into_masechet = ttk.Checkbutton(
            self.merge_options_frame, text="Merge entire selection into one PDF", variable=self.merge_into_one_pdf_var)
        self.merge_into_masechet.grid(column=1)
        self.merge_into_masechet.bind()
        
        select_type = self.select_type_var.get()
        self.merge_into_dapim_var = tk.BooleanVar(value=False)
        self.merge_into_dapim_check = ttk.Checkbutton(self.merge_options_frame, text=str, variable=self.merge_into_dapim_var)
        ...
        self.download_button.grid(pady=20, column=1,row=6,sticky="S")
    ...
    def update_individual_list(self):
        ...
        if mode != "Dapim":
            self.dapim_var = ("Merge Amudim into Dapim PDFs")
        # Update scroll region after adding widgets
        self.individual_list_frame.update_idletasks()
        self.individual_canvas.configure(scrollregion=self.individual_canvas.bbox("all"))
    ...
    def toggle_selection_widgets(self):
        ...
        if mode == "Range":
            ...
            if select_type == "Dapim":
                self.merge_into_dapim_check.configure(text="Keep the Dapim as Separate PDF Files")
            elif select_type == "Amudim":
                self.merge_into_dapim_check.configure(text="Merge Range into Dapim PDFs (in Development)")
            print("Got to Range")
        elif mode == "Individual":
            print("Got to Individual")
            if select_type == "Dapim":
                self.merge_into_dapim_check.configure(text="Merge into sets of 2s (I will hopefully make this better soon)")
            elif select_type == "Amudim":
                self.merge_into_dapim_check.configure(text="Merge into Dapim PDFs (This will merge the downloaded PDFs into sets of 2 in order. \nThis forces it, even if you only selecterd separate amudim.)")
            self.individual_list_frame_container.grid(column=0,row=3,pady=5, sticky=("W","E","N"), padx=10)
            self.individual_list_frame_container.grid_anchor("nw")
            self.update_individual_list() # Ensure list is populated correctly
        elif mode == "All":
            if select_type == "Dapim":
                self.merge_into_dapim_check.configure(text="Keep Dapim PDFs")
            elif select_type == "Amudim":
                self.merge_into_dapim_check.configure(text="Merge into Dapim PDFs")
            print("Got here")
            #self.dapim_var = ("Keep Dapim PDFs")
    ...
    def start_download(self):
        ...
        if selection_mode == "All":
            ...
            selected_items = [item for item, var in self.individual_vars.items() if var.get()]
        ...
        if merge_amudim_into_dapim:
            ...
            #daf_files[0].append("file0")
            #daf_files.("file1")
            print(downloaded_amud_files.items())
            for page_num, filepath in downloaded_amud_files.items():
                daf, amud = MasechetDownloader.daf_amud_calculator2(page_num)
                if daf not in daf_files: daf_files[daf] = []
                print(daf,amud,filepath)
                #this next line needs a lot of work
                daf_files[daf].append(filepath)
            print(daf_files.keys())
            print(daf_files.items())
